...
trait HubspotTrait
{
    ...
    protected function createHBDeal($dealInfo)
    {
        ...
        $dealstage_id = $dealInfo['dealstage_id'] ? $dealInfo['dealstage_id'] : "";
        $closedate = "";
    ...
    protected function getHubspotContactEmails($nextMaxHubspotObjectId, $requestData){
        $client = new \GuzzleHttp\Client();
        $apiUrl = '';
        $properties = 'hs_email_from_email,hs_email_to_email,hs_email_text,hs_timestamp,hubspot_owner_id,hs_email_direction,
        hs_email_status,hs_email_subject,hs_attachment_ids,hs_email_from_firstname,hs_email_from_lastname,hs_email_to_firstname,hs_email_to_lastname';

        $method = 'POST';
        $apiUrl = "https://api.hubapi.com/crm/v3/objects/emails/search";
        $appToken = "";
        $payload = [
            ...
            'filterGroups' => [
                [
                    'filters' => [
                        [
                            'highValue' => $requestData['end_date'],
        ...
        if($nextMaxHubspotObjectId){
            $payload['after'] = $nextMaxHubspotObjectId;
        }
        try {
    ...
    protected function getHubspotContactNotes($nextMaxHubspotObjectId){
        $client = new \GuzzleHttp\Client();
        $apiUrl = '';
        $properties = 'hs_timestamp,hs_note_body,hubspot_owner_id,hs_attachment_ids';
        $association = 'Contacts,Companies,Deals,Tickets,Calls,Quotes,Meetings,Products,Feedback_submissions';
        $method = 'GET';
        $apiUrl = "https://api.hubapi.com/crm/v3/objects/notes?limit=60&associations={$association}&properties={$properties}";
        $appToken = "";
        if($nextMaxHubspotObjectId){
            $apiUrl.=  "&after={$nextMaxHubspotObjectId}";
        }
        try {
            $response = $client->request($method, $apiUrl, [
                'headers' => [
    ...
    protected function getHubspotTask($nextMaxHubspotObjectId){
        $client = new \GuzzleHttp\Client();
        $apiUrl = '';
        $properties = 'hs_timestamp,hs_task_body,hubspot_owner_id,hs_task_subject,hs_task_status,hs_task_priority,hs_task_type,hs_task_reminders';
        $association = 'Contacts,Companies,Deals,Tickets,Calls,Quotes,Meetings,Products,Feedback_submissions';
        $method = 'GET';
        $apiUrl = "https://api.hubapi.com/crm/v3/objects/tasks?limit=60&associations={$association}&properties={$properties}";
        if($nextMaxHubspotObjectId){
            $apiUrl.=  "&after={$nextMaxHubspotObjectId}";
        }
        $appToken = "";
