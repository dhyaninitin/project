...
trait PortalTraits
{
    ...
    protected function addKeyToCollection($collection, $key, $keyValue){
        $collection->each(function ($item) use ($key, $keyValue){
            // Add your key-value pair to each item
            $item->setAttribute($key, $keyValue);
            if ($item->offsetExists('hs_timestamp')) {
                $value = $item->offsetGet('hs_timestamp');
                $item->offsetUnset('hs_timestamp');
                $item->setAttribute('created_at', $value);
            }
        });
        return $collection;
    }

    protected function arrayShorting($array){
        usort($array, function($a, $b) {
            $carbonA = Carbon::parse($a['created_at'])->setTimezone('America/Los_Angeles');
            $carbonB = Carbon::parse($b['created_at'])->setTimezone('America/Los_Angeles');
            return $carbonB->timestamp - $carbonA->timestamp;
        });
        return $array;
    }

    protected function attachmentIds($emailArray){
        $getAttachmentIds = $emailArray->filter(function ($item) {
            return !empty($item['hs_attachment_ids']);
        })->pluck('hs_attachment_ids')->flatMap(function ($item) {
            return explode(';', $item);
        })->values()->all();
        return $getAttachmentIds;
    }

    protected function mapAttachmentWithEmail($emailArray, $attachmentArray){
        if(!empty($emailArray) && $emailArray->isNotEmpty()){
            if(!empty($attachmentArray) && $attachmentArray->isNotEmpty()){
                $filterEmailArray = $emailArray->filter(function ($item) {
                    return !empty($item['hs_attachment_ids']);
                });

                foreach ($filterEmailArray as $key => $emailValue) {
                    if(isset($emailValue->hs_attachment_ids)){
                        $attachmentId = explode(';',$emailValue->hs_attachment_ids);
                        $attachmetFiles = $attachmentArray->filter(function ($item) use ($attachmentId) {
                            return in_array($item->attachment_id, $attachmentId);
                        });
                        $fileArray=[];
                        foreach ($attachmetFiles as $key => $value) {
                            $temp = [];
                            $filepath = config('services.hs_client_file.bucket')."/".config('services.hs_client_file.folder')."/{$value->path}"."/{$value->name}".".{$value->extension}";
                            $fileUrl = Storage::disk('s3')->temporaryUrl($filepath, now()->addMinutes(5));
                            $temp['file_name'] = $value->name;
                            $temp['extension'] = $value->extension;
                            $temp['file_url'] = $fileUrl;
                            array_push($fileArray,$temp);
                        }
                        $emailValue['files'] = $fileArray;
                    }
                }
            }
            return $emailArray;
    ...
    protected function addCallRecordingUrl($callResult){
        if(!empty($callResult) && $callResult->isNotEmpty()){
            foreach ($callResult as $key => $callValue) {
                if(isset($callValue->recording)){
                    $temp = [];
                    $filepath = config('services.hs_client_file.bucket')."/calls"."/{$callValue->recording}";
                    $fileUrl = Storage::disk('s3')->temporaryUrl($filepath, now()->addMinutes(5));
                    $callValue['recording_url'] = $fileUrl;
                }
            }
        }
        return $callResult;
