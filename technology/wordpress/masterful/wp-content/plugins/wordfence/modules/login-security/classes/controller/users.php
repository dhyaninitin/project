...
class Controller_Users {
	...
	/**
	 * Imports the array of 2FA s. Users that do not currently exist or are disallowed from enabling 2FA are not imported.
	 *
	 * @param array $s An array of s in the format array(<user id> => array('secret' => <secret in hex>, 'recovery' => <recovery keys in hex>, 'ctime' => <timestamp>, 'vtime' => <timestamp>, 'type' => <type>), ...)
	 * @return int The number imported.
	 */
	public function import_2fa($s) {
		global $wpdb;
		$table = Controller_DB::shared()->s;
		
		$count = 0;
		foreach ($s as $id => $parameters) {
			$user = new \WP_User($id);
...
SQL;
		...
		$query = <<<SQL
			...
			WHERE
				...
				AND NOT $usermeta.user_id IN(SELECT user_id FROM {$s})
SQL;
		$conditions = array();
		$operator = 'AND';
		if ($gracePeriod !== null) {
