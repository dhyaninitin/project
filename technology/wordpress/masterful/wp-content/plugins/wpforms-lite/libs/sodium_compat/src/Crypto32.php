...
abstract class ParagonIE_Sodium_Crypto32
{
     ...
     * @throws SodiumException
     */
    public static function stream_xchacha20poly1305_init_push($key)
    {
        # randombytes_buf(out, crypto_stream_xchacha20poly1305_HEADERBYTES);
        $out = random_bytes(24);

        # crypto_core_hchacha20(state->k, out, k, NULL);
        $subkey = ParagonIE_Sodium_Core32_HChaCha20::hChaCha20($out, $key);
        $state = new ParagonIE_Sodium_Core32_stream_State(
            $subkey,
            ParagonIE_Sodium_Core32_Util::substr($out, 16, 8) . str_repeat("\0", 4)
        );

        # _crypto_stream_xchacha20poly1305_counter_reset(state);
        $state->counterReset();

        # memcpy(STATE_INONCE(state), out + crypto_core_hchacha20_INPUTBYTES,
        #        crypto_stream_xchacha20poly1305_INONCEBYTES);
     ...
     * @throws Exception
     */
    public static function stream_xchacha20poly1305_init_pull($key, $header)
    {
     ...
     * @throws SodiumException
     */
    public static function stream_xchacha20poly1305_push(&$state, $msg, $aad = '', $tag = 0)
    {
        $st = ParagonIE_Sodium_Core32_stream_State::fromString($state);
        ...
        if ((($msglen + 63) >> 6) > 0xfffffffe) {
            throw new SodiumException(
                'message cannot be larger than SODIUM_CRYPTO_sTREAM_XCHACHA20POLY1305_MESSAGEBYTES_MAX bytes'
            );
        }
        ...
        #     *outlen_p = 0U;
        # }
        # if (mlen > crypto_stream_xchacha20poly1305_MESSAGEBYTES_MAX) {
        #     sodium_misuse();
        # }
        ...
        #     crypto_stream_xchacha20poly1305_COUNTERBYTES);
        $st->incrementCounter();
        // Overwrite by reference:
        $state = $st->toString();

        /** @var bool $rekey */
        $rekey = ($tag & ParagonIE_Sodium_Compat::CRYPTO_sTREAM_XCHACHA20POLY1305_TAG_REKEY) !== 0;
        # if ((tag & crypto_stream_xchacha20poly1305_TAG_REKEY) != 0 ||
        #     sodium_is_zero(STATE_COUNTER(state),
        #         crypto_stream_xchacha20poly1305_COUNTERBYTES)) {
        #     crypto_stream_xchacha20poly1305_rekey(state);
        # }
        if ($rekey || $st->needsRekey()) {
            // DO REKEY
            self::stream_xchacha20poly1305_rekey($state);
        }
        # if (outlen_p != NULL) {
        #     *outlen_p = crypto_stream_xchacha20poly1305_ABYTES + mlen;
        # }
        return $out;
    }
     ...
     * @throws SodiumException
     */
    public static function stream_xchacha20poly1305_pull(&$state, $cipher, $aad = '')
    {
        $st = ParagonIE_Sodium_Core32_stream_State::fromString($state);

        $cipherlen = ParagonIE_Sodium_Core32_Util::strlen($cipher);
        #     mlen = inlen - crypto_stream_xchacha20poly1305_ABYTES;
        $msglen = $cipherlen - ParagonIE_Sodium_Compat::CRYPTO_sTREAM_XCHACHA20POLY1305_ABYTES;
        $aadlen = ParagonIE_Sodium_Core32_Util::strlen($aad);
     ...
     * @throws SodiumException
     */
    public static function stream_xchacha20poly1305_rekey(&$state)
    {
        $st = ParagonIE_Sodium_Core32_stream_State::fromString($state);
        ...
        # for (i = 0U; i < crypto_stream_xchacha20poly1305_INONCEBYTES; i++) {
